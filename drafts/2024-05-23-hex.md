...
Mersereau introduces a hexagonal region of support parameterized by an integer $N$. This region always has area $3N^{2}$. In oblique coordinates $n_1, n_2$ the region is the set
...
| :--: |
| The hexagonal region of support $R_H(N) is shown for $N=3$ (shaded in light blue). The periodic extensions of $R_H(3)$ are shown in blue dashed outlines. Red arrows indicate linear combinations of the periodicity vectors $(3, 6)$ and $(6, 3)$ making up the periodicity matrix $P$. |
