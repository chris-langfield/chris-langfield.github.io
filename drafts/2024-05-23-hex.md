...
Where $n_1, n_2 \in R_H(N)$ and $k_1, k_2$ also $\in R_H(N)$, i.e. the frequency values are sampled on the same gridpoints. This last expression is the form given by Mersereau in the paper. The problem, which Mersereau calls an "insurmountable" difficulty, is that unlike the 2D FFT in rectilinear coordinates, this kernel is not separable. Therefore it is not possible, in this coordinate system, to reduce the 2D transform to two 1D FFTs. Through a clever resampling of $R_H(N)$ and splitting the problem into smaller hexagonal "slow" DFTs, a speed-up from $3N^{2}$ to $9N^{2}/4$ complex multiplications is achieved by the FFT described by Mersereau. However, the algorithm also adds overhead due to pre-computation of phase shift matrices, also scaling as $N^2$. I will leave a detailed discussion of the fast algorithm and its time/space complexity for a future post.
...
The figure above demonstrates why this periodicity matrix was chosen. A parallelogram in physical space is not the most convenient region of support for real-life signals. What we really want is a rectangular region. The region shaded in light blue above, $R_R(6, 8)$ can be re-sampled horizontally by moving the right upper triangular region onto the right side, creating a rectangular region (in pink) whose periodicity is rectangular. One periodic extension of this re-sampled region is shown outlined in red. 
...
The time complexity is thus $N_1 N_2 \log N_1 N_2$.
